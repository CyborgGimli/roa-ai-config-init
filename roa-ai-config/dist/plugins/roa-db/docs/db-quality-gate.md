...
[gate] mvn test -Pe2e -Dtest=AccountQueryTests   -> exit 0   PASS  (7 tests, 0 failures)
...
```text
[gate] mvn clean compile                        -> exit 0   PASS
[gate] mvn test -Pe2e -Dtest=AccountQueryTests   -> exit 1   FAIL
         2 failures, 0 errors
         AccountQueryTests.searchByName_withNoMatch_showsEmptyState
...
```bash
mvn test -Pe2e -Dtest=AccountQueryTests          # during the change
