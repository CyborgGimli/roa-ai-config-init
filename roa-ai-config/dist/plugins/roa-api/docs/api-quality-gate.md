# ROA API Testing — Quality Gate Examples

Worked examples of the gate defined in `quality-gates.md`, as `roa-api-quality-gate`
runs it.
...
[gate] mvn test -Pe2e -Dtest=ProfileApiTests   -> exit 0   PASS  (7 tests, 0 failures)
...
```text
[gate] mvn clean compile                        -> exit 0   PASS
[gate] mvn test -Pe2e -Dtest=ProfileApiTests   -> exit 1   FAIL
         2 failures, 0 errors
         ProfileApiTests.searchByName_withNoMatch_showsEmptyState
...
```bash
mvn test -Pe2e -Dtest=ProfileApiTests          # during the change
