---
name: roa-ui-quality-gate
description: The reusable UI verification gate - runs the applicable build and tests for this repository and reports explicit pass/fail evidence. Consumed by validate-code.
user-invocable: false
allowed-tools: Read, Grep, Glob, Bash
---

This hidden skill is the single UI gate consumed by every ROA workflow.
...
- **Conditional check selection** - do not run every tool unconditionally. Decide from:
  - repo config (`pom.xml` plugins/profiles, `ai-config.yaml`, `.claude/rules/roa-ui.md`),
...
- **Candidate checks** (run only when applicable):
  ...
  - **Metadata** - `mvn pandora:open -U` when dependencies or framework version changed.
